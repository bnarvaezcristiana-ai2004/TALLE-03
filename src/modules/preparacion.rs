    ...
    let df = LazyCsvReader::new(&config.input_path)
        ...
        .with_column(
            col("created_at")
                ...
                .to_datetime(
                    None, None,
                    StrptimeOptions { format: Some("%Y-%m-%d %H:%M:%S".into()), ..Default::default() },
                    lit("raise"),
                )
                .alias("fecha_parsed")
        )
        .with_column(col("fecha_parsed").dt().date().alias("fecha_dia"))
        .collect()
        .map_err(GasAnalysisError::PolarsError)?;

    let file = std::fs::File::create(&config.output_path).map_err(GasAnalysisError::IoError)?;
    ParquetWriter::new(file)
        .with_compression(ParquetCompression::Zstd(None))  // ✅ Zstd con 's' minúscula
        .finish(&mut df.clone())
        .map_err(GasAnalysisError::PolarsError)?;

    info!("✅ Listo: {} filas", df.height());
    Ok(())
}